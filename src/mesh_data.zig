...
pub fn loadOptimizedMesh(allocator: std.mem.Allocator, cpuPrimitive: *ModelLoader.CPUPrimitive, all_primitives: *std.ArrayList(Primitive), all_vertices: *std.ArrayList(Vertex), all_indices: *std.ArrayList(u32), all_meshlets: *std.ArrayList(Meshlet), all_meshlets_data: *std.ArrayList(u32)) !void {
    ...
    try remap.resize(cpuPrimitive.indices.items.len);

    const num_unique_vertices = zmesh.opt.generateVertexRemap(remap.items, cpuPrimitive.indices.items, Vertex, cpuPrimitive.vertices.items);

    var optimized_vertices = std.ArrayList(Vertex).init(arenaAllocator);
    try optimized_vertices.resize(num_unique_vertices);

    zmesh.opt.remapVertexBuffer(Vertex, optimized_vertices.items, cpuPrimitive.vertices.items, remap.items);

    var optimized_indices = std.ArrayList(u32).init(arenaAllocator);
    try optimized_indices.resize(cpuPrimitive.indices.items.len);
    zmesh.opt.remapIndexBuffer(optimized_indices.items, cpuPrimitive.indices.items, remap.items);
